...
const options = {
  main_plugin_route: './plugins/controller',
  address: '0.0.0.0',
  ports: {
    query: -1,
    cache: -1,
...
if (options.ports.controller === -1) {
  throw new Error("Invalid port %d", options.ports.portCache);
  process.exit(-1);
}

fastify.register(require(options.main_plugin_route), options);

fastify.listen(options.ports.controller, options.address, function (err, address) {
  if (err) {
